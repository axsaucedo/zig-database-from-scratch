...
//! Demo of internal node searching
...
pub fn main() !void {
    ...
    if (args.len < 2) {
        std.debug.print("Usage: db_phase11 <filename>\n", .{});
        return;
    }

    std.debug.print("Phase 11: Recursively Searching the B-Tree\n", .{});
    std.debug.print("Insert now works across internal nodes!\n\n", .{});
    ...
    while (true) {
        ...
        if (phase02.isMetaCommand(input)) {
            ...
            if (std.mem.eql(u8, input, ".btree")) {
                ...
                try phase11.printTree(&table, table.root_page_num, 0);
