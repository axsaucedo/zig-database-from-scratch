...
//! Demo of complete B-Tree with internal node splitting
...
pub fn main() !void {
    ...
    if (args.len < 2) {
        std.debug.print("Usage: db_phase14 <filename>\n", .{});
        return;
    }

    std.debug.print("Phase 14: Splitting Internal Nodes\n", .{});
    std.debug.print("Complete B-Tree implementation!\n\n", .{});
    ...
    while (true) {
        ...
        if (phase02.isMetaCommand(input)) {
            ...
            if (std.mem.eql(u8, input, ".btree")) {
                ...
                try phase14.printTreeSafe(&table, table.root_page_num, 0);
